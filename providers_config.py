...
def get_data_dir(name: str) -> Path | None:
    ...
    If env_home is set and the env var exists, use it as base.
    Otherwise expand ~ in data_dir.
    """
    provider = get_provider(name)
    if not provider:
        return None
    ...
    if provider.env_home:
        home_override = os.environ.get(provider.env_home, "").strip()
        if home_override:
            return Path(home_override) / "token-telemetry"
