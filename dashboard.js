const TRANSLATIONS = {
      en: {
        ...
        // Layers table
        honestMeasurement: "Honest measurement: parent chat excluded from blend. Legacy = old KPI (often <1%).",
        layer: "Layer",
        savings: "Savings",
        observed: "Observed",
        pctLayer: "% layer",
        ...
        gainPerDayFootnote: "% = savings ÷ (observed + savings), aggregated per day (RTK + Task compression + Diff-Only). Readable even when Cursor billing reaches several million tokens.",
        observedVsOptTitle: "Observed vs without optimizations",
        observedVsOptFootnote: "Observed: <code>billed_total_tokens</code> (agent responses), tool/subagent proxies, compressed prompt. Savings: <code>rtk gain -d</code> (per day), Task compression, Diff-Only chars ÷ 4. In hourly mode, the daily RTK gain is distributed across active hours.",
        ...
        noProvidersMessage: "Enable at least one telemetry provider in your .env file, then restart the server.",
      ...
      fr: {
        ...
        // Layers table
        honestMeasurement: "Mesure honnête : chat parent exclu du blend. Legacy = ancien KPI (souvent <1%).",
        layer: "Couche",
        savings: "Économies",
        observed: "Observé",
        pctLayer: "% couche",
        ...
        gainPerDayFootnote: "% = économies ÷ (observé + économies), agrégé par jour (RTK + compression Task + Diff-Only). Lisible même quand la facturation de Cursor atteint plusieurs millions de tokens.",
        observedVsOptTitle: "Observé vs sans optimisations",
        observedVsOptFootnote: "Observé : <code>billed_total_tokens</code> (réponses agent), proxies outil/sous-agent, prompt complet. Économies : <code>rtk gain -d</code> (par jour), compression Task, caractères Diff-Only ÷ 4. En mode horaire, le gain RTK quotidien est réparti sur les heures actives.",
        distributionTitle: "Distribution (proxy)",
        ...
        noProvidersError: "Aucun provider activé. Définir TELEMETRY_*_ENABLED=1 dans .env (voir .env.example)",
        noProvidersTitle: "Aucun provider configuré",
        noProvidersMessage: "Activez au moins un provider de télémétrie dans votre fichier .env, puis redémarrez le serveur.",
    ...
    function destroyCharts() {
      ...
      if (gainPctChart) { gainPctChart.destroy(); gainPctChart = null; }
    ...
    function buildConsumptionComparison(events, mode, rtkGain) {
      ...
      const savingsData = labels.map(
        (l) => Math.round((hookAndDiff[l] || 0) + (rtkInBuckets[l] || 0)),
      );
    ...
    function renderLayerKpis(layerPayload) {
      ...
      rows.forEach(([key, label]) => {
        ...
        const avail = L.available === false ? (getLanguage() === "fr" ? " (RTK désactivé)" : " (RTK off)") : "";
    ...
    function renderCharts(events, bucketMode, isDark, rtkGain) {
      ...
      const useLogGainAxis =
        maxGainPct > 5 && minGainPct > 0 && maxGainPct / minGainPct >= 15;
      const linearGainMax =
        maxGainPct <= 0
          ? 10
          : maxGainPct < 20
            ? Math.ceil(maxGainPct * 1.3 * 10) / 10
            : 100;
      ...
      trendChart = new Chart(ctxTrend, {
        ...
        options: {
          ...
          scales: {
            x: {
              ticks: { color: tick, maxRotation: 45, minRotation: 42, font: { size: 9, family: "'Plus Jakarta Sans'" } },
              grid: { color: grid },
              border: { display: false },
            },
            y: {
    ...
    function renderEditStats(events) {
      ...
      events.forEach((e) => {
        ...
        const isEditEvent = e.event === "afterFileEdit" ||
          (e.event === "postToolUse" && (e.tool === "Edit" || e.tool === "Write") && (e.lines_added || e.lines_removed));
        if (isEditEvent) {
    ...
    function renderConsumptionStats(events) {
      ...
      if (!total) {
        ...
        document.getElementById("kpiConsoSub").textContent = t("completeFields", { pct: c.complete_pct });
      }
    }
    ...
    function renderComplianceStats(events) {
      ...
      document.getElementById("kpiConsoCompleteSub").textContent =
        c.responses > 0 ? t("completeHookChecks", { pct: c.complete_pct, checks: fmtNum(c.hook_checks) }) : "afterAgentResponse";
      ...
      document.getElementById("kpiConsoHookSub").textContent =
        t("consoHookSub", { ok: fmtNum(c.hook_ok), giveups: fmtNum(c.hook_giveups) });
    ...
    function renderComplianceTable(events) {
      ...
      const rows = events
        .filter(
          (e) =>
              e.event === "taskBriefValidation" || e.event === "consumptionReportCompliance",
        )
      ...
      rows.forEach((e) => {
        ...
        if (e.event === "taskBriefValidation") {
          ...
          cDetail.textContent = t("complianceLoopEnforced", { loop: e.loop_count ?? "?", enforced: e.consumption_enforced === true });
        }
    ...
    function renderCrgTable(events) {
      ...
      document.getElementById("kpiCrgRisk").textContent = allCrg.length > 0 ? (crgRiskSum / allCrg.length).toFixed(2) : "—";
      ...
      if (rowIdempotentInjected(launch)) {
        parts.push(
          makeOptBadge(
            "Idempotent",
            "idempotent",
            "Brief with [IDEMPOTENT_CONTEXT_INJECTED]",
          ),
        );
      }
      if (!parts.length) {
    ...
    function renderStackKpis(events) {
      ...
      document.getElementById("kpiGitCacheSub").textContent =
        s.launches > 0
          ? t("gitCacheHitsSub", { hits: s.gitHits, launches: s.launches, preserved: fmtCompact(s.gitPreserved) })
          : t("noTaskLaunches");
    ...
    function eventsForCurrentTour(events) {
      ...
      return events.filter(
        (e) => e.session_id === key || e.conversation_id === key,
      );
    }
    ...
    function applyReportSummary(summary) {
      ...
      document.getElementById("kpiSubStopSub").textContent =
        sub.stop > 0
          ? t("subStopSubVal", { hook: fmtNum(sub.stop_hook || 0), fallback: fmtNum(sub.stop_post_tool_fallback || 0) })
      ...
      if (billed.count) {
        ...
        document.getElementById("kpiParentBilledSub").textContent =
          t("avgSumLatest", {
      ...
      document.getElementById("kpiSubPromptSub").textContent = t("launchCount", { count: fmtNum(sub.launch || 0) });
      document.getElementById("kpiSubOutSub").textContent = t("stopCount", { count: fmtNum(sub.stop || 0) });
    }
    ...
    function renderHookCompressionStats(events) {
      ...
      if (!count) {
        ...
        const backendHint = Object.entries(backends)
          ...
          .map(([k, n]) => `${k}×${n}`)
          .join(" · ");
        document.getElementById("kpiHookSub").textContent =
          t("hookSummarySub", {
            count: fmtNum(count),
    ...
    function renderRtkGainStats(rtkGain, hookSavedTokens) {
      ...
      if (p.ok === true && typeof ps.total_saved === "number") {
        ...
        subEl.textContent = t("gProjects", { gPct: gpct, gSaved: fmtCompact(gSaved), pPct: ppct, pSaved: fmtCompact(pSaved) });
    ...
    function updateNavPeriod(events, overrideText) {
      ...
      const dates = events
        ...
        .sort((a, b) => a - b);
      if (dates.length) {
        el.setAttribute("datetime", `${dates[0].toISOString()}/${dates[dates.length - 1].toISOString()}`);
    ...
    function sectionDropTarget(container, clientY, dragging) {
      const sections = [...container.querySelectorAll(".dash-section")].filter(
        (s) => s !== dragging,
      );
    ...
    function syncRefreshMenuRadios(activeMs) {
      ...
      const label = document.querySelector(
        `#refreshDropdown button[data-interval-ms="${activeMs}"]`,
      );
