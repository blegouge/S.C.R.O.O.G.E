...
- **S.C.R.O.O.G.E** (`~/www/private/SCROOGE`) + **`hooks/token-telemetry.py`**: proxy sizes (`postToolUse`, `afterAgentResponse`) plus **LOC heuristics** (`afterFileEdit`, `afterTabFileEdit`). Composer refus = non disponible via hooks publics → **`SCROOGE/README.md`**. UI: **`serve_dashboard.py`** (navigateur) ou **`dashboard_app.py`** + `pywebview` ; données persistantes dans **`~/.cursor/token-telemetry/`** (`events.jsonl`, layout, cache Diff-Only).
